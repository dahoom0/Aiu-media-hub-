  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // ✅ C. TUTORIALS LOGIC (FIXED: show real progress, including 100%)
        ...
        // 2) Continue Learning list (include completed too, sorted by last_watched_at)
        const progressBasedList = Array.isArray(progressData)
          ? progressData
              .map((p: any) => {
                const percent = toNumberPercent(p.progress_percentage);
                if (percent <= 0) return null; // ignore totally empty
        ...
        // Fallback ONLY if user never started anything
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Card className="bg-gray-900/50 border-gray-800">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className={theme === 'light' ? 'text-slate-900' : 'text-white'}>Continue Learning</CardTitle>
              ...
              recentTutorials.map((tutorial) => (
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      <p className={theme === 'light' ? 'text-slate-900 hover:text-teal-600 transition-colors line-clamp-1' : 'text-white hover:text-teal-400 transition-colors line-clamp-1'}>
                        {tutorial.title}
                      </p>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className={theme === 'light' ? 'text-slate-900' : 'text-white'}>{tutorial.progress}%</span>
                    </div>
                    <Progress
