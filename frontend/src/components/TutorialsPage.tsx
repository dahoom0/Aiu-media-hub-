...
import { Search, Play, Eye, Filter, Loader2, CheckCircle2 } from 'lucide-react';
...
export function TutorialsPage({ onNavigate }: TutorialsPageProps) {
  ...
  const fetchData = async () => {
    try {
      ...
      const tList = (tData as any).results || tData;
      const pList = (pData as any).results || pData;

      // Map: tutorial_id -> { id, percentage, completed }
      const pMap: Record<number, any> = {};
      (pList as any[]).forEach((p: any) => {
        pMap[p.tutorial] = {
          id: p.id,
      ...
      // Normalize tutorials for UI
      const mappedTutorials = (tList as any[]).map((t: any) => ({
        id: t.id,
        title: t.title,
  ...
  // --- RENDER VIDEO PLAYER ---
  if (selectedVideo) {
    const progressInfo = progressMap[selectedVideo.id] || {};
    return (
      <VideoPlayerPage
        video={selectedVideo}
        initialProgress={progressInfo.percentage || 0}
        isInitiallyCompleted={progressInfo.completed || false}
        onBack={() => {
          setSelectedVideo(null);
          fetchData(); // refresh list after watching
        }}
      />
    );
  }
  ...
  const categories = [
    { id: 'all', label: 'All Tutorials', count: tutorials.length },
    {
      ...
      count: tutorials.filter((t) =>
        ['camera', 'audio', 'lighting', 'equipment'].some((c) =>
          t.category.includes(c),
        ),
      ).length,
    },
    {
      ...
      count: tutorials.filter(
        (t) =>
          t.category.includes('general') || t.category.includes('editing'),
      ).length,
    },
  ];
  ...
  const filteredTutorials = tutorials.filter((tutorial) => {
    ...
    if (!matchesCategory) {
      if (selectedCategory === 'equipment') {
        matchesCategory = ['camera', 'audio', 'lighting', 'equipment'].some(
          (c) => tutorial.category.includes(c),
        );
      } else {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        >
          Learn how to use professional media equipment with our comprehensive
          guides
        </p>
      </div>
      ...
      <Tabs value={selectedCategory} onValueChange={setSelectedCategory}>
        <TabsList
          className={`border w-full justify-start overflow-x-auto ${
            theme === 'light'
        ...
        <TabsContent value={selectedCategory} className="mt-6">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filteredTutorials.map((tutorial) => {
              const progressInfo = progressMap[tutorial.id] || {};
              const progress = progressInfo.percentage || 0;
